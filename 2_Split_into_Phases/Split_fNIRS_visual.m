...
%       • Bottom: chosen fNIRS channels overlaid, with the meditation
%         window highlighted on the same time axis. Optional per-channel
%         z-score normalization.
%   How to use:
%       After processing, confirm the prompt → pick one processed folder
%       (must contain fNIRS_phases.mat) → select fNIRS channels to show
...
for f = 1:length(folders)
    ...
    ts_gsr = datetime(gsr_data(:,1) / 1000, 'ConvertFrom','posixtime', ...
                      'Format','yyyy-MM-dd HH:mm:ss.SSS');
    gsr_values = gsr_data(:,2); %#ok<NASGU>
    ...
    before_meditation = fNIRS_data(:, ts_fnirs <  t_start_meditation);
    meditation        = fNIRS_data(:, ts_fnirs >= t_start_meditation & ts_fnirs <= t_end_meditation);
    after_meditation  = fNIRS_data(:, ts_fnirs >  t_end_meditation);
...
%% === OPTIONAL: one-off segmentation viewer (GSR + fNIRS) ================
resp = questdlg('Show a segmentation visualization now?', ...
                'Segmentation Viewer', 'Yes','No','Yes');
if strcmp(resp,'Yes')
    try
        % find folders with fNIRS_phases.mat
        cand = dir(fullfile(out_root, '**', 'fNIRS_phases.mat'));
        vis_folders = unique({cand.folder});
        if isempty(vis_folders)
            errordlg('No folders with fNIRS_phases.mat found under out_root.','No Data');
        else
            % pick one folder
            ...
            if ok
                ...
                S = load(fullfile(input_folder,'fNIRS_filtered.mat'),'fnirs_filtered');
                nCh = size(S.fnirs_filtered,1);
                ch_ans = inputdlg({sprintf('fNIRS channels to show (e.g. 1,3,5 or 1:4 or all), 1..%d:', nCh), ...
                                   'Normalize per-channel (0/1):'}, ...
                                   'Viewer options', 1, {'1:4','0'});
                if ~isempty(ch_ans)
                    ch_list = parse_channels_local(ch_ans{1}, nCh);
                    do_norm = str2double(ch_ans{2})~=0;
                    show_segmentation_figure_fnirs(input_folder, ch_list, do_norm);
                end
            end
        end
    catch ME
...
function show_segmentation_figure_fnirs(input_folder, ch_list, do_norm)
Fs_fNIRS = 250;
...
% align fNIRS time to GSR end (same logic as in segmentation)
end_time = ts_gsr(end);
t_fnirs  = end_time - seconds((size(FN,2)-1)/Fs_fNIRS) + seconds((0:size(FN,2)-1)/Fs_fNIRS);

% optional z-score per channel
if do_norm
    mu = mean(FN,2,'omitnan'); sd = std(FN,0,2,'omitnan'); sd(sd==0)=1;
    FN = (FN - mu)./sd;
...
ch_list = ch_list(ch_list>=1 & ch_list<=size(FN,1));
if isempty(ch_list), ch_list = 1; end

% figure
f = figure('Name','GSR + fNIRS segmentation','Color','w');
...
for i = 1:numel(ch_list)
    ch = ch_list(i);
    plot(ax2, t_fnirs, FN(ch,:), 'Color', colors(i,:), ...
        'DisplayName', sprintf('fNIRS ch %d', ch));
end
ylabel(ax2,'fNIRS'); xlabel(ax2,'Time');
title(ax2,'fNIRS with meditation window');
